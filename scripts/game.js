function startNewGame() {
  ...
  gameFieldElement.style.display = 'block'
}
...
function resetGameStatus() {
  ...
  for (let i = 0; i < 3; i++) {
    for (let j = 0; j < 3; j++) {
      const gameBoardItemElement = gameFieldItems[gameBoardIndex]
      gameData[i][j] = 0
...
function checkForGameOver() {
  ...
  // Checking the Diogonal: Top left to bottom right
  if (
    gameData[0][0] > 0 &&
  ...
  // Checking the Diogonal: Bottom left to Top right
  if (
    gameData[2][0] > 0 &&
